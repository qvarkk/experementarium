...
import {Lab, APIResponse} from "../../Lib/types";

export const getLabs = async (): Promise<AxiosResponse<APIResponse<Lab[]>>> => {
    return await api.get('/labs')
}

export const getLab = async (id: number): Promise<AxiosResponse<APIResponse<Lab>>> => {
    return await api.get(`/labs/${id}`)
}

export const createLab = async (labData: Omit<Lab, 'id'>): Promise<AxiosResponse<APIResponse<Lab>>> => {
